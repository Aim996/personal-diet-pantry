...
def test_packed_installable_starts_without_source_tree(
    ...
    assert package["version"] == "0.9.4"
    assert package["productVersion"] == "0.7.5.4"
    assert (
    ...
    assert (installed / "UPDATE-v0.7.5.4.zh-CN.md").is_file()
    assert (
    ...
    assert imported.stdout.strip() == "0.9.4"
