...
def test_readme_is_user_first_and_keeps_the_protected_receipt() -> None:
    ...
    assert "Personal Diet Pantry v0.7.5" in readme
    assert "技术包版本 `0.9.0`" in readme
...
def test_current_release_documents_state_migration_and_candidate_boundary() -> None:
    update = text("UPDATE-v0.7.5.zh-CN.md")
    ...
    assert release.startswith("# 食序管家（Personal Diet Pantry）v0.7.5\n")
    assert "personal-diet-pantry-0.7.5-installable.tgz" in release
    assert "本地开发候选" in release


def test_changelog_uses_product_versions_and_migration_labels() -> None:
    changelog = text("CHANGELOG.md")
    assert "## [0.7.5]" in changelog
...
def test_install_upgrade_and_release_entries_are_exact() -> None:
    ...
    assert "personal-diet-pantry-0.7.5-installable.tgz" in install
    ...
    assert "git ls-remote --tags origin refs/tags/v0.7.5" in releasing
...
def test_ai_prompts_are_complete_and_have_no_placeholders() -> None:
    ...
    assert "personal-diet-pantry-0.7.5-installable.tgz" in prompts
