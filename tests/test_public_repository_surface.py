...
def test_readme_is_user_first_and_keeps_the_protected_receipt() -> None:
    ...
    assert "Personal Diet Pantry v0.7.4.27" in readme
    assert "技术包版本 `0.8.27`" in readme
...
def test_current_release_documents_state_no_migration_or_remote_release() -> None:
    update = text("UPDATE-v0.7.4.27.zh-CN.md")
    release = text("RELEASE.zh-CN.md")
    for phrase in (
        "没有新增 migration",
        "0.7.4.19",
        "继续使用 migrations 001–021",
        "不会自动推送 GitHub、创建 Tag 或 Release",
        "可食部（玉米粒）约90克（估算）",
    ):
        assert phrase in update
    assert release.startswith("# 食序管家（Personal Diet Pantry）v0.7.4.27\n")
    assert "personal-diet-pantry-0.7.4.27-installable.tgz" in release


def test_changelog_uses_product_versions_and_migration_labels() -> None:
    changelog = text("CHANGELOG.md")
    assert "## [0.7.4.27]" in changelog
...
def test_install_upgrade_and_release_entries_are_exact() -> None:
    ...
    assert "personal-diet-pantry-0.7.4.27-installable.tgz" in install
    ...
    assert "git ls-remote --tags origin refs/tags/v0.7.4.27" in releasing
...
def test_ai_prompts_are_complete_and_have_no_placeholders() -> None:
    ...
    assert "personal-diet-pantry-0.7.4.27-installable.tgz" in prompts
    for forbidden in (
        "<项目",
        "<版本",
        "TBD",
