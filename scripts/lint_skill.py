#!/usr/bin/env python3
"""Deterministically lint progressive-disclosure Skill routing fixtures."""
...
def lint_skill(skill_root: Path, cases_path: Path) -> LintResult:
    """Lint case schema, one-level routing, safety, and referenced contracts."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--skill",
        type=Path,
        default=Path("skills/personal-diet-pantry"),
    )
    parser.add_argument(
        "--cases",
        type=Path,
        default=Path("tests/skill-evals/routing.yaml"),
    )
    args = parser.parse_args()
    result = lint_skill(args.skill, args.cases)
    print(
