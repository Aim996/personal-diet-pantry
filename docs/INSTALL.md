...
这是给普通 OpenClaw 用户和代为执行安装的智能体使用的简明入口。v0.7.5.4 正式发布后，固定下载页为 [GitHub Release v0.7.5.4](https://github.com/Aim996/personal-diet-pantry/releases/tag/v0.7.5.4)；页面不存在时说明尚未发布，不得从不明来源替代。完整步骤见[详细安装手册](INSTALLATION.zh-CN.md)。
...
只从 `Aim996/personal-diet-pantry` 的正式 v0.7.5.4 GitHub Release 下载：

- `personal-diet-pantry-0.7.5.4-installable.tgz`
- `SHA256SUMS`

不要安装 `personal-diet-pantry-0.7.5.4-source.tar.gz`；源码包只用于审阅和复现。不要从聊天附件、网盘或不明镜像获取安装包。
...
```powershell
Get-FileHash -Algorithm SHA256 -LiteralPath '.\personal-diet-pantry-0.7.5.4-installable.tgz'
...
如果 OpenClaw 位于 Docker、远程网关或自定义服务管理器中，使用该实例既有的配置和重启方式，不要在无法确认目标实例时照抄本机命令。Docker 前台 Gateway 的容器内 `openclaw gateway restart` 可能不会真的重启；必须核对重启前后 PID 或启动时间已经变化，否则仍按“未完成”处理。Python 优先由 OpenClaw 进程的 `PYTHON` 环境变量选择；未设置时，Windows 默认使用 `python`，其他平台默认使用 `python3`。插件没有 `pythonExecutable` 配置字段。
...
5. 记录产品版本 `0.7.5.4`、技术包版本 `0.9.4`、七类工具状态、自检结果和 `dataDir` 是否独立，不记录数据库内容。

`self_check` 不能证明其他六类工具已经注册，因此第 1 步不可省略。任何一步失败都应明确报告“未完成”，再查阅[故障排除](TROUBLESHOOTING.zh-CN.md)。

## 7. 安全失败与回滚

全新安装失败时保留 `dataDir`，不要反复初始化或删除数据库。从 v0.7.5.3 更新到本版不新增 migration，仍应先建立经校验的冷备份；普通回退可重装 v0.7.5.3 并复用相同的 023 schema，数据完整性异常时则先恢复冷备份。任何哈希、真实进程重启、工具注册、自检或记录数量证据缺失时，都应报告“未完成”。
