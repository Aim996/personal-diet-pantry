...
这是给普通 OpenClaw 用户和代为执行安装的智能体使用的简明入口。固定下载页是 [GitHub Release v0.7.4.28](https://github.com/Aim996/personal-diet-pantry/releases/tag/v0.7.4.28)。完整的隔离、初始化、冷备份和故障处理命令见[详细安装手册](INSTALLATION.zh-CN.md)。
...
只从 `Aim996/personal-diet-pantry` 的正式 v0.7.4.28 GitHub Release 下载：

- `personal-diet-pantry-0.7.4.28-installable.tgz`
- `SHA256SUMS`

不要安装 `personal-diet-pantry-0.7.4.28-source.tar.gz`；源码包只用于审阅和复现。不要从聊天附件、网盘或不明镜像获取安装包。
...
```powershell
Get-FileHash -Algorithm SHA256 -LiteralPath '.\personal-diet-pantry-0.7.4.28-installable.tgz'
...
5. 记录产品版本 `0.7.4.28`、技术包版本 `0.8.28`、七类工具状态、自检结果和 `dataDir` 是否独立，不记录数据库内容。

`self_check` 不能证明其他六类工具已经注册，因此第 1 步不可省略。任何一步失败都应明确报告“未完成”，再查阅[故障排除](TROUBLESHOOTING.zh-CN.md)。

## 7. 安全失败与回滚

全新安装失败时保留 `dataDir`，不要反复初始化或删除数据库。更新现有实例前必须先停止实例并建立经校验的升级前冷备份；本版没有新增 migration，失败时可按[安全更新与回滚](UPGRADING.md)恢复已校验的 v0.7.4.19 技术包或冷备份。任何哈希、工具注册、自检或记录数量证据缺失时，都应报告“未完成”，不能宣称成功。
