...
这是给普通 OpenClaw 用户和代为执行安装的智能体使用的简明入口。v0.7.5.0 正式发布后，固定下载页为 [GitHub Release v0.7.5.0](https://github.com/Aim996/personal-diet-pantry/releases/tag/v0.7.5.0)；页面不存在时说明尚未发布，不得从不明来源替代。完整步骤见[详细安装手册](INSTALLATION.zh-CN.md)。
...
只从 `Aim996/personal-diet-pantry` 的正式 v0.7.5.0 GitHub Release 下载：

- `personal-diet-pantry-0.7.5.0-installable.tgz`
- `SHA256SUMS`

不要安装 `personal-diet-pantry-0.7.5.0-source.tar.gz`；源码包只用于审阅和复现。不要从聊天附件、网盘或不明镜像获取安装包。
...
```powershell
Get-FileHash -Algorithm SHA256 -LiteralPath '.\personal-diet-pantry-0.7.5.0-installable.tgz'
...
5. 记录产品版本 `0.7.5.0`、技术包版本 `0.9.1`、七类工具状态、自检结果和 `dataDir` 是否独立，不记录数据库内容。
