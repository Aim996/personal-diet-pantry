...
如果 OpenClaw 位于 Docker、远程网关或自定义服务管理器中，使用该实例既有的配置和重启方式，不要在无法确认目标实例时照抄本机命令。Python 优先由 OpenClaw 进程的 `PYTHON` 环境变量选择；未设置时，Windows 默认使用 `python`，其他平台默认使用 `python3`。插件没有 `pythonExecutable` 配置字段。
