...
适用版本：Personal Diet Pantry（食序管家）v0.7.5.3。
...
| 恢复后验证 | 停止实例后，按[成套回滚流程](INSTALLATION.zh-CN.md#7-成套回滚)处理。v0.7.5.3 新增 migration 023；回退到 v0.7.5.2 时必须恢复升级前冷备份，不能只换旧代码并复用已迁移数据库。在线 `diet_system backup` 仍仅用于同版本恢复。启动后运行 `self_check`，再以 `diet_pantry query`、`diet_report progress` 做只读验证。 |
...
| 恢复后验证 | 注册故障应使用 `personal-diet-pantry-0.7.5.3-installable.tgz` 的 npm-pack 安装路径重装同版本，并保留当前专用 `dataDir` 映射后重新加载。七类工具出现后调用 `diet_system self_check`。若需要回退，按[成套回滚流程](INSTALLATION.zh-CN.md#7-成套回滚)恢复升级前冷备份，再安装 v0.7.5.2。 |

## 营养估算失败

| 项目 | 处置 |
| --- | --- |
| 现象 | 饮食记录的部分营养字段为 `null`，并以 `nutrition_data_state` 或 `nutrition_status` 表示不完整。v0.7.5.3 继续允许可靠标签缺失的字段保持未知，不应为通过写入而补造零值或模型估算。 |
