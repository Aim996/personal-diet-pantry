...
本页只供维护者发布食序管家 v0.7.5。版本目录和制品一经创建即不可变；发现已有同名目录、Tag 或 Release 时必须停止，不得覆盖。
...
2. Confirm productVersion 0.7.5 and package version 0.9.0.
3. Run `git ls-remote --tags origin refs/tags/v0.7.5`; stop if output is non-empty.
...
正式 Tag 必须是已批准提交上的 annotated tag `v0.7.5`。Release 工作流的手动触发只能做 dry-run 构建；只有推送精确匹配产品版本、且提交可追溯到 `origin/main` 的 Tag 才能进入 `GitHub Release` 发布 job。发布前先确认 `gh release view v0.7.5` 不存在；存在即停止，不得覆盖、替换或补传同名制品。
