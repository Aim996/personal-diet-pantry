...
本文描述从产品 v0.7.4.28 更新到 v0.7.5 的最小安全顺序。v0.7.5 新增 migration 022，因此程序包与数据库必须成套升级、成套回退。
...
4. 取得 `personal-diet-pantry-0.7.5-installable.tgz` 和 `SHA256SUMS`，核对同名文件 SHA-256。若 v0.7.5 Release 尚未创建，只能把本地制品当作候选测试包，不能冒充公开正式版。

## 安装更新

1. 保持实例停止，保留原外部 `dataDir`，不要手改 `diet.sqlite`、WAL 或 `schema_migrations`。
2. 通过 `openclaw plugins install npm-pack:` 安装已校验的 v0.7.5 可安装包。
...
3. 用只读方式确认产品版本 `0.7.5`、技术包版本 `0.9.0`、migration 最大版本 022，并比较关键记录数量。
