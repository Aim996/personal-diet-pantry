...
Personal Diet Pantry `v0.7.5.0` is a local-first OpenClaw skill for one person's meals, nutrition, hydration, body weight, pantry, cooking, leftovers, corrections, and reports. The technical package version is `0.9.1`. SQLite in an external `dataDir` is the formal source of truth.

## Current status

This tree is a local v0.7.5.0 development candidate. It has not yet created a Git tag, GitHub Release, or OpenClaw deployment; build and publication do not auto-install, enable, restart, or configure any instance.
...
```text
openclaw plugins install npm-pack:/path/to/personal-diet-pantry-0.7.5.0-installable.tgz
