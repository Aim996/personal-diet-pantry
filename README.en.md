...
Personal Diet Pantry `v0.7.4.27` is a local-first OpenClaw skill for one person's meals, nutrition, hydration, body weight, pantry, cooking, leftovers, corrections, and reports. The technical package version is `0.8.27`. SQLite in an external `dataDir` is the formal source of truth.

## Current status

This repository is still preparing the v0.7.4.27 release assets. It remains Private, and no Git tag or GitHub Release has been created. The repository does not auto-install, enable, restart, or configure any OpenClaw instance.
...
After an official Release exists, verify `SHA256SUMS` and install only `personal-diet-pantry-0.7.4.27-installable.tgz` through OpenClaw's npm-pack path. The source archive is for review and reproduction, not plugin installation.

```text
openclaw plugins install npm-pack:/path/to/personal-diet-pantry-0.7.4.27-installable.tgz
```

Requirements: OpenClaw `>=2026.5.17`, Node.js `>=22.22.3`, Python `>=3.11,<4`, and a dedicated persistent `dataDir` outside the source checkout. After installation, independently confirm all seven tools, then run initialization and self-check. See [the concise install guide](docs/INSTALL.md) and [the detailed Chinese installation manual](docs/INSTALLATION.zh-CN.md).
